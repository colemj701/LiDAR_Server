...
def bld_tile_path(contains,DQ):
    ...
    if len(mismatch) > 0:
        log_message(f'The following tiles were not found within {gdb}:\n   ::  {un_match}')
...
def build_tile_index(in_feat, Tile_Field,DQ,root):
    ...
    for _, row in df.iterrows():
        ...
        tile_list.append(f_index_item)
        index_list.append(ras_path)
